...
class CropBordersStep(BaseProcessingModule):
    def action(self):
        ...
        for band in bands:
            if band == f'{self.channel_names[0]}_irradiancee_RGB':
                img_dir = os.path.join(self.DIR, band, 'undistord_solving')
            else:
                img_dir = os.path.join(self.DIR, band, 'align')
